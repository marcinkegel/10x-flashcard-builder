...
3.  **Logika Ochrony (Guard):**
    ...
    *   **Scenariusz 1 (Brak sesji):** Jeśli URL *nie* jest w `PUBLIC_ROUTES`, przekieruj na `/login`.
    *   **Scenariusz 2 (Jest sesja):** Jeśli URL *jest* w `PUBLIC_ROUTES` (np. użytkownik wchodzi na login będąc zalogowanym), przekieruj na `/generate`.
    *   **Scenariusz 3 (Root `/`):**
