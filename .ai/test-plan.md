...
- **Implementacja testowa:**
  ...
  describe("RLS Security", () => {
    ...
    it("should prevent cross-user data access", async () => {
      ...
      const { data, error } = await supabase.from("flashcards").select("*").eq("user_id", "user-b"); // Próba dostępu do danych innego użytkownika
...
describe("GenerationService", () => {
  ...
  describe("checkDuplicate", () => {
    it("should return true when duplicate exists", async () => {
      ...
      const isDuplicate = await GenerationService["checkDuplicate"](mockSupabase, userId, hash);
    ...
    it("should return false when no duplicate found", async () => {
      ...
      const isDuplicate = await GenerationService["checkDuplicate"](mockSupabase, userId, "new-hash");
  ...
  describe("generateFlashcards", () => {
    it("should throw error for duplicate content", async () => {
      ...
      vi.spyOn(GenerationService as any, "checkDuplicate").mockResolvedValue(true);

      await expect(GenerationService.generateFlashcards(mockSupabase, userId, command)).rejects.toMatchObject({
...
test.describe("Complete generation flow", () => {
  ...
  test("should generate flashcards from text", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="proposal-item"]').first()).toBeVisible({ timeout: 30000 });
    ...
    await page.locator('[data-testid="proposal-item"]').first().locator('button[aria-label*="Edytuj"]').click();
    ...
    await expect(page.locator('[role="status"]')).toContainText(/zapisano/i, { timeout: 5000 });
  ...
  test("should show error for duplicate generation", async ({ page }) => {
    ...
    await expect(page.locator('[role="alert"]')).toContainText(/przetworzony/i, { timeout: 5000 });
