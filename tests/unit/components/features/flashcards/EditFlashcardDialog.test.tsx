...
describe('EditFlashcardDialog', () => {
  ...
  it('calls API and triggers onSuccess on successful update', async () => {
    ...
    await waitFor(() => {
      expect(mockFetch).toHaveBeenCalledWith('/api/flashcards/f-1', expect.objectContaining({
        method: 'PUT',
        body: JSON.stringify({
          front: 'Updated Front',
          back: 'Original Back',
  ...
  it('updates source to "ai-edited" if original source was "ai-full" and content changed', async () => {
    ...
    mockFetch.mockResolvedValueOnce({
      json: async () => ({ success: true, data: { ...aiFlashcard, front: 'Original Front (edited)', source: 'ai-edited' } }),
    });
