...
export const StudyCard: React.FC<StudyCardProps> = ({ front, back, isFlipped, onClick }) => {
  return (
    <div 
      className="group perspective-1000 w-full max-w-[min(90vw,400px)] aspect-[3/4] sm:aspect-[4/5] md:aspect-[3/4] cursor-pointer"
      onClick={onClick}
    >
      <div 
        className={cn(
        ...
        <div className="absolute inset-0 backface-hidden">
          <Card className="w-full h-full flex items-center justify-center p-8 text-center bg-card border-2">
            <CardContent className="flex flex-col items-center justify-center h-full pt-6 px-4 sm:px-8">
              <span className="text-xs sm:text-sm font-medium text-muted-foreground mb-4 opacity-70 tracking-widest uppercase">FRONT</span>
